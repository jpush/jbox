...
public class MainActivity extends FragmentActivity
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
      ...
      @Override public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
          case MotionEvent.ACTION_DOWN:
            LogUtil.LOGI(TAG, "mDrawerLayout touch down");
      ...
      public void onScroll(AbsListView absListView, int firstVisibleItem, int visibleItemCount,
        ...
        if (mMsgListView.getLastVisiblePosition() + 1 == totalItemCount) {
          ...
          List<Message> newMessageList =
              queryMessages(AppApplication.currentDevKey, AppApplication.currentChannelName,
                  mMessages.size(), QUERY_MESSAGE_COUNT);
    ...
    mMessages = queryMessages(AppApplication.currentDevKey, AppApplication.currentChannelName, 0,
        QUERY_MESSAGE_COUNT);
  ...
  @Override public void onChannelListItemClick(Channel channel) {
    mTopBar.setTitle(channel.name);

    // Team 栏对应图标上减去未读消息数。
    if (channel.unreadCount != 0) {
      Developer dev =
          new Select().from(Developer.class).where("Key=?", channel.devKey).executeSingle();
    ...
    List<Message> messages =
        queryMessages(channel.devKey, channel.name, 0, mCurrentOffset + QUERY_MESSAGE_COUNT);
  ...
  @Override public void onPageSelected(boolean isLast) {
    if (isLast) {
      mDrawerLayout.setDrawerLockMode(DrawerLayout.LOCK_MODE_UNLOCKED);
    } else if (mDrawerLayout.getDrawerLockMode(GravityCompat.START)
